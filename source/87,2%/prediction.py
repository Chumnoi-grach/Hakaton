...
def create_student_features(df):
    ...
    for student_id in df['PK'].unique():
        ...
        features['gpa_drop_abs'] = abs(features['course_1_avg'] - features['course_2_avg'])
        features['gpa_drop_bin'] = 1 if features['gpa_drop_abs'] > 1.0 else 0

        features['sem_drop_abs'] = abs(features['sem_4_avg'] - features['sem_3_avg'])
        features['sem_drop_bin'] = 1 if features['sem_drop_abs'] > 1.0 else 0

        features['nd'] = student_data['MARK'].eq('нд').sum()
...
feature_columns = [
    ...
    'gpa_drop_abs', 'gpa_drop_bin',
    'nd',
    'sem_drop_abs', 'sem_drop_bin'
]
